public class B3_HalfTheRed extends World {
    public int red;
    public int blue;
    public int green;

    public void go() {
        System.out.println("This will be printed to the  window. ");
        plane.teleport(0, 0);
        plane.showBackGround();
        System.out.println(plane.howMuchRed());

        for (int row = 1; row < 750; row = row + 1) {
            for (int col = 1; col < 901; col++) {//"++" is shorthand for "+1"
                plane.teleport(col, row);

                red = plane.howMuchRed();
                green = plane.howMuchGreen();
                blue = plane.howMuchBlue();
if(red>50 && green>60 && green<200 && col>453){
    plane.setPixelColor(240, row/3,col/4);
}
if(red>30 && blue>10 && blue<200 && green>240 && col<454){
    plane.setPixelColor(col/4, row/3,160);
}
if(green>60 && red>50 && red<256 && row>439){
    plane.setPixelColor(100, 210,row/3);
}
            }
        }

        plane.teleport(38,43);
        star();
        plane.teleport(136,36);
        star();
        plane.teleport(56,116);
        star();
        plane.teleport(240,51);
        star();
        plane.teleport(68,69);
        star();
        plane.teleport(25,181);
        star();
        plane.teleport(290,31);
        star();
        plane.teleport(648,40);
        star();
        plane.teleport(837,43);
        star();
        plane.teleport(846,163);
        star();
        plane.teleport(815,142);
        star();
        plane.teleport(707,28);
        star();
        plane.teleport(790,66);
        star();
        plane.teleport(338,21);
        star();
        plane.teleport(551,23);
        star();
        plane.teleport(844,101);
        star();
        plane.teleport(434,14);
        star();

        plane.teleport(72,508);
        flower();
        plane.teleport(51,566);
        stem();
        plane.teleport(232,559);
        flower();
        plane.teleport(213,616);
        stem();
        plane.teleport(642,503);
        flower();
        plane.teleport(621,560);
        stem();
        plane.teleport(810,558);
        flower();
        plane.teleport(790,615);
        stem();
    }
    public void star(){
       plane.isTrail=true;
       plane.setColor(255,255,255);
        plane.startingAngle(225);
        plane.turn(345);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
        plane.move(10);
        plane.turn(120);
        plane.move(10);
        plane.turn(15);
    }
    public void flower(){
        plane.setColor(255,204,0);
        plane.startingAngle(225);
        plane.trailWidth=10;
        plane.turn(345);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
        plane.move(50);
        plane.turn(135);
        plane.move(50);
        plane.turn(15);
    }
    public void stem(){
        plane.setColor(0,255,0);
        plane.isTrail=true;
        plane.trailWidth=10;
        plane.startingAngle(90);
        plane.move(500);
    }

}


/* 

// Below is a list of the variables in plane and how to set them:

   plane.anglefacing=90 ;     // Sets the angle the plane is facing
	plane.xpos=100;            // Sets the xposition of the plane
	plane.ypos=100;            // Sets the yposition of the plane
   plane.isTrail=false;       // Sets if the plane has a trail
   plane.trailWidth=1;        // Sets the width of the trail from the plane
   plane.pausetime=1;         // Sets how long the plane pauses each time it moves, 
                                 make this bigger to watch the drawing happen
   
// Below are all the methods the plane can use:

   System.out.println("fun" + variable);  // Prints text to the window + the value of the variable
   plane.drawWords("this is fun");  // Prints text to the applet at the location of the plane
   plane.circle(10);                // Draws a circle of radius 10
   plane.square(50);                // Draws a square with side length 50, 
                                      the plane finishes in the same direction it starts  
   plane.house(100);                // Draws a house with a bottom edge of 100
   plane.fillCircle(100);           // Draws a circle of radius 100 and fills it in
   plane.move(100);                 // Moves the plane forward (the direction it is facing) 100 pixels
   plane.teleport(111, 222);        // Relocates the plan to the x position 111 and the y position 222, 
                                      there is no trail with teleport
   plane.turn(90);                  // Turns the plan 90 degrees counterclockwise
   plane.setColor(255, 255, 255);   // Sets the trail color Red, Green, Blue; max value of 255, min 0
   plane.setPixelColor(255, 255, 255);  // Sets the pixel color at the plane's location to Red, Green, Blue; max value of 255, min 0
   plane.howMuchBlue();             // Returns a number between 0 and 255 describing how much blue there is
   plane.howMuchGreen();            // Returns a number between 0 and 255 describing how much green there is
   plane.howMuchRed();              // Returns a number between 0 and 255 describing how much red there is
   plane.random(0, 100);            // Returns an integer on the interval 0 to 99
   
*/